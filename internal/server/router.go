...
func NewRouter(h *Handler) http.Handler {
	...
	mux.Handle("/d3/post", h.RequiredAuthentication(http.HandlerFunc(h.Post)))                  // 200    	(GET METHOD) post and his comments
	...
	mux.Handle("/userd3/post-delete", h.RequiredAuthentication(http.HandlerFunc(h.DeletePost))) // 202   // (DELETE METHOD) delete

	mux.Handle("/d3/post-like", h.RequiredAuthentication(http.HandlerFunc(h.LikePosts)))           // 200  // (POST METHOD)
	mux.Handle("/userd3/comment-like", h.RequiredAuthentication(http.HandlerFunc(h.LikeComments))) // 200  // (POST METHOD)
